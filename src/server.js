...
const updateDatabase = (reportRequest, database) => {
    for (const report of database){
        if(report.city == reportRequest.city && report.date == reportRequest.date){
            report.temperature = reportRequest.temperature;
            report.unit = reportRequest.unit;
            return true;
        }
    }
    return false;    
...
app.post("/api/reports", async (req, res) => {
    ...
    const updatedDatabase = updateDatabase(sanitizedFields, db.reports);
